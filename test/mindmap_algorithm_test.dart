...
void main() {
  group('MindMap Graph', () {
    ...
    test('MindMap Node positions are correct for Top_Bottom', () {
      ...
      // MindMap layout should arrange nodes in a balanced way around the root
      expect(graph.getNodeAtPosition(0).position.dx, isNotNull);
      expect(graph.getNodeAtPosition(0).position.dy, isNotNull);
      expect(size.width > 0, true);
      expect(size.height > 0, true);
    });

    test('MindMap detects cyclic dependencies', () {
